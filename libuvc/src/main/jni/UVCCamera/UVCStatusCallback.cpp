...
void UVCStatusCallback::notifyStatusCallback(JNIEnv *env, uvc_status_class status_class, int event, int selector,
    ...
    pthread_mutex_lock(&status_mutex);
    {
        if (mStatusCallbackObj) {
            ...
            env->CallVoidMethod(mStatusCallbackObj, istatuscallback_fields.onStatus, (int) status_class, event,
                                selector, (int) status_attribute, buf);
            env->ExceptionClear();
...
void UVCStatusCallback::uvc_status_callback(uvc_status_class status_class, int event,
                                            int selector,
